import { del, get, post, sleep } from "./ajax";
...
const AnimePicturesDirect = {
    ...
    async addTags(tagNames: string, postId: number, createTags: boolean = false): Promise<string> {
        const res: PostHtmlTags = await post(`${HOST}/api/v3/posts/${postId}/tags`, {
            query: tagNames,
            add_new_tag: createTags,
        });
    ...
    async removeTag(tagId: number, postId: number): Promise<string> {
        const res: PostHtmlTags = await del(`${HOST}/api/v3/posts/${postId}/tags/${tagId}`);
        return res.post_tags;
    },
    /**
...
const AnimePictures = new Proxy(AnimePicturesDirect, {
    get(_, prop) {
        ...
        return async (...args: any[]) => {
            ...
            const result = Reflect.apply(fn, AnimePictures, args);

            const delay = sleep(used >= LIMIT ? MINUTE : ((used / LIMIT) * 2 * MINUTE) / LIMIT);
            Promise.allSettled([result, delay]).then(resolve);
            return result;
        };
    },
});
