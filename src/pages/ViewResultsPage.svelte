<script lang="ts">
    ...
    const search: SavedResult = (params.name === "search"
        ? storage.search
        : storage[`res_${params.name}`]
    ) ?? {} as any;
...
<header>
    ...
    {#if search.results[0]?.sim}
        <label>
            <input type="checkbox" bind:checked={storage.showSource}>
            show the source image,
        </label>
    {/if}
    post size: <select bind:value={storage.postSize}>
...
<div class="posts" style:--post-size="{storage.postSize}px">
    {#each posts as post (post.result.id)}
        {#if storage.showSource && canShowSource}
            <SimilarityPost {post} multiAction={multiAction} {dataProvider} />
        {:else}
            <Post post={post.result} multiAction={multiAction} />
        {/if}
    {/each}
</div>
<PageNavigator {baseUrl} {currPage} {pageCount} />
